...
void ofApp::update() {
	...
	for (float r = R * 0.125; r <= R * 0.25; r += 2) {

		ofColor face_color = flag ? ofColor(239) : ofColor(39);
		ofColor line_color = flag ? ofColor(39) : ofColor(239);
		flag = !flag;

		for (float phi_deg = -10; phi_deg < 360; phi_deg += phi_deg_step) {

			for (float theta_deg = 0; theta_deg < 360; theta_deg += theta_deg_step) {

				auto noise_value = ofNoise(glm::vec4(this->make_point(R, 10, theta_deg, phi_deg) * 0.02, r + ofGetFrameNum() * 0.005));
				if (noise_value <= threshold_1 || noise_value >= threshold_2) { continue; }

				auto noise_1 = ofNoise(glm::vec4(this->make_point(R, 10, theta_deg - theta_deg_step, phi_deg) * 0.02, r + ofGetFrameNum() * 0.005));
				auto noise_2 = ofNoise(glm::vec4(this->make_point(R, 10, theta_deg, phi_deg + phi_deg_step) * 0.02, r + ofGetFrameNum() * 0.005));
				auto noise_3 = ofNoise(glm::vec4(this->make_point(R, 10, theta_deg, phi_deg - phi_deg_step) * 0.02, r + ofGetFrameNum() * 0.005));
				auto noise_4 = ofNoise(glm::vec4(this->make_point(R, 10, theta_deg + theta_deg_step, phi_deg) * 0.02, r + ofGetFrameNum() * 0.005));
